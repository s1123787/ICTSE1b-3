...
namespace KBSGame
{
    public class Obstacle
    {
        ...
        {  
            ...
            while ((x <= 100 && y <= 100) || (x >= 650 && y >= 450) || Obstacles.values.Contains($"{x}{y}b") || Obstacles.values.Contains($"{x}{y}t") || Obstacles.values.Contains($"{x}{y}m") || Obstacles.values.Contains($"{x}{y}c"))
            {
                x = random.Next(0, 15) * 50;
                y = random.Next(0, 11) * 50;
            }
            //add the position to the list where all obstacles are placed
            Obstacles.values.Add($"{x}{y}{soort}");
